#include "ev3api.h"
#include "app.h"
#include "linetrace.h"

#if defined(BUILD_MODULE)
#include "module_cfg.h"
#else
#include "kernel_cfg.h"
#endif
// グローバル変数の宣言


// ライントレース処理の実装
void line_trace() {

    int turn=0;
    int B=45; //turn to power
    int W=45;
    int power=18; 
    int now_val=0;
    int now_color =0;
    int front_color =0;
    float p = 1.2;

    const int threshold = 42;

    now_val = line_ref;
    now_color = line_cor;
    front_color= color_flag;


    if (status == GO_STORE) {

        if (now_color == 2) {
            cyc_cnt++;
        }else{
            cyc_cnt = 0;
        }
        if (cyc_cnt > 6) { //stop
            power=0;
            turn=0;
            cyc_cnt = 0;
            status = PARKING;
        }


        if (front_color == 3 || front_color == 5) { // 緑または赤を検出
            stop_count++;
        } else {
            stop_count = 0; // リセット
        }

        if (stop_count == 1) { // 停止条件
            power = 0;
            turn = 0;
            stop_count = 0;
            last_color = front_color;
            status = PICK_UP; // i think here
        } else {
            if (now_val < threshold) {
                turn = p * (now_val - threshold); // P制御
            } else {
                turn = p * (now_val - threshold); // P制御
            }
        }

        // カラーセンサーが緑または赤を検出した場合,直進する
        if(now_color == 5 || now_color == 3){
            turn = 0;
        }
       
    }

    if (status == GO_YARD) {

        if (front_color != 3 && front_color != 5) { // 緑または赤を未検出
            stop_count++;
        } else if (now_color == last_color) { // 緑または赤を検出
            stop_count++;
        } else {
            stop_count = 0; // リセット
        }

        if (stop_count == 3 && (front_color == 3 || front_color == 5)) { // カウンターが3になり、緑または赤を検出
            power = 0;
            turn = 0;
            stop_count = 0;
            status = DROP_DOWN;
        
        } else if (stop_count == 3 && (front_color != 3 && front_color != 5)) { // カウンターが3になり、緑または赤を未検出
            power = 0;
            turn = 0;
            stop_count = 0;
            last_status = status;   // 状態を保存
            status = ERROR;         // エラー状態に遷移

        } else {
            if (now_val < threshold) {
                turn = p * (now_val - threshold); // P制御
            } else {
                turn = p * (now_val - threshold); // P制御
            }
        }
    }

    if (status == GO_HOME) {
        if (now_color == 2) {
            stop_count++;
        } else {
            stop_count = 0; // リセット
        }

        if (stop_count == 4) { // 停止条件
            power = 0;
            turn = 0;
            stop_count = 0;
            status = LOOP_WAIT;
        
        } else {
            if (now_val < threshold) {
                turn = p * (now_val - threshold); // P制御
            } else {
                turn = p * (now_val - threshold); // P制御
            }
        }
    }

    ev3_motor_steer(L_motor, R_motor, power, turn);
}